...
function StepThree({ onNext, onBack }) {
  ...
  useEffect(() => {
    ...
    if (isMediating && meditationTime > 0) {
      timer = setTimeout(() => setMeditationTime(meditationTime - 1), 1000);
  ...
  return (
    <div className="step-three">
      ...
      <div className="button-group">
        ...
        <button onClick={handleSubmit} className="next-button"><i className="fas fa-paper-plane icon"></i>Submit and Continue</button>
      </div>
    </div>
  );
}
